...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--ckpt", required=False, type=str, default='/public_bme/data/lifeng/code/moco/DiffBIR-main/weights/lightning_logs/version_1352278/checkpoints/step=1299.ckpt', help="full checkpoint path") #/public_bme/data/lifeng/code/moco/DiffBIR-main/checkpoints/cldm.ckpt
    """
    baseline: .../version_1329720/checkpoints/step=5149.ckpt
    """
    parser.add_argument("--config", required=False, type=str, default='/public_bme/data/lifeng/code/moco/DiffBIR-main/configs/model/cldm.yaml', help="model config path")
    parser.add_argument("--reload_swinir", action="store_true", default=True)
    parser.add_argument("--swinir_ckpt", type=str, default="/public_bme/data/lifeng/code/moco/DiffBIR-main/checkpoints/lightning_logs/version_1285486/checkpoints/step=999.ckpt")
    
    parser.add_argument("--input", type=str, default='/public_bme/data/lifeng/code/moco/DiffBIR-main/inputs/demo/T1_brain_input', required=False)
    ...
    parser.add_argument("--disable_preprocess_model", action="store_true",default=True)  #测试用lq去guide,因此preprocess为disable
    ...
    parser.add_argument("--g_scale", type=float, default=100)
    ...
    parser.add_argument("--output", type=str, required=False, default='/public_bme/data/lifeng/code/moco/DiffBIR-main/inputs/demo/T1_brain_input')
...
def main() -> None:
    ...
    for file_path in list_image_files_natural(args.input, follow_links=True):
        ...
        for i in range(args.repeat_times):
            ...
            pnsr = calculate_psnr_pt(pred_2, gt_2, crop_border=0).mean()
            lpips = lpips_metric(pred_, gt_, normalize=True).mean()  #0.2679
            ssim = calculate_ssim_pt(pred_2, gt_2, crop_border=0).mean()  #0.7713
            
            index = file_path.split('/')[-1].split('_')[0]
            index_A = file_path.split('/')[-1].split('_')[1]
            res_save = (einops.rearrange(pred, "c h w -> h w c") * 255).clip(0, 255).astype(np.uint8)
            pred_filename = "{}_idx-{}_slice-{}_pnsr-{}_ssim-{}_lpips-sample_diffbir.png".format(index, index_A, pnsr, ssim, lpips)
            pred_path = os.path.join(args.input, pred_filename)
            Image.fromarray(res_save).save(pred_path)
            # if args.show_lq:
