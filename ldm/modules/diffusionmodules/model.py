...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # downsampling
        hs = [self.conv_in(x)]   #[6, 3, 512, 512] -> [6, 128, 512, 512]
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
                h = self.down[i_level].block[i_block](hs[-1], temb)
                if len(self.down[i_level].attn) > 0:
        ...
        h = self.mid.block_1(h, temb)
        h = self.mid.attn_1(h)
        h = self.mid.block_2(h, temb)  #[6, 512, 64, 64]

        # end
        h = self.norm_out(h)     #[6, 512, 64, 64]
        h = nonlinearity(h)
        h = self.conv_out(h)    # [6, 512, 64, 64] -> [6, 8, 64, 64]
        return h
