...
class SpacedSampler:
    ...
    def apply_cond_fn(
        ...
        for _ in range(cond_fn.repeat):
            ...
            delta_pred = cond_fn(target, pred, t_now)   #loss的梯度
    ...
    def p_sample(
        self,
        x: torch.Tensor,   #输入的x_t
        cond: Dict[str, torch.Tensor],      #c_latent
        t: torch.Tensor,
        index: torch.Tensor,
        cfg_scale: float,
        uncond: Optional[Dict[str, torch.Tensor]],
        cond_fn: Optional[Guidance]        #mseguidance
    ) -> torch.Tensor:
    ...
    def sample(
        ...
        self.make_schedule(num_steps=steps)  #从1000steps中选择20个steps,计算好各种超参数的值
        ...
        for i, step in enumerate(iterator):    #迭代steps的次数
            ts = torch.full((b,), step, device=device, dtype=torch.long)  #torch.full(size, fill_value），创建一个矩阵元素全为fill_value的大小为size的tensor,比如b=2, step = 999, 则ts-> (999,999)
            index = torch.full_like(ts, fill_value=total_steps - i - 1)    #i=0, totol_steps=20, 则index -> (19,19)
            img = self.p_sample(
        ...
        img_pixel = (self.model.decode_first_stage(img) + 1) / 2                          #将gt的（-1,1）转换到(0,1)
        # apply color correction (borrowed from StableSR)
        if color_fix_type == "adain":
            ...
            assert color_fix_type == "none", f"unexpected color fix type: {color_fix_type}"
        #return img_pixel     
        return img_pixel#, img      #多返回了一个latent