...
#from tem_attention import T_BMIR, Fusion_ST
import numpy as np
...
class SwinIR(pl.LightningModule, ImageLoggerMixin):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  #[1, 3, 512, 512]*3, [B,6,3,512,512]
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            #-----start-----#
            x = x.view(-1, C, H, W)   
            x = self.conv_first(x).view(B, D, self.embed_dim, int(H/self.upscale), int(W/self.upscale))  
            # #-----end-----#
            x_key = x[:,2,...]  
            tempotal_x = self.temporal_block(x)  
            x = self.forward_features(x_key,tempotal_x)    
            
            x = self.conv_after_body(x) + x_key  
